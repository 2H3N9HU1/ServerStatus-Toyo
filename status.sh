...
Download_Server_Status_server(){
	cd "/tmp"
	wget -N --no-check-certificate "https://mirror.ghproxy.com/https://github.com/2H3N9HU1/ServerStatus-Toyo/archive/master.zip"
...
Download_Server_Status_client(){
	cd "/tmp"
	wget -N --no-check-certificate "https://mirror.ghproxy.com/https://raw.githubusercontent.com/2H3N9HU1/ServerStatus-Toyo/master/clients/status-client.py"
...
Service_Server_Status_server(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "https://mirror.ghproxy.com/https://raw.githubusercontent.com/2H3N9HU1/doubi/master/service/server_status_server_centos" -O /etc/init.d/status-server; then
		...
		if ! wget --no-check-certificate "https://mirror.ghproxy.com/https://raw.githubusercontent.com/2H3N9HU1/doubi/master/service/server_status_server_debian" -O /etc/init.d/status-server; then
			echo -e "${Error} ServerStatus 服务端服务管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/status-server
...
Service_Server_Status_client(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "https://mirror.ghproxy.com/https://raw.githubusercontent.com/2H3N9HU1/doubi/master/service/server_status_client_centos" -O /etc/init.d/status-client; then
		...
		if ! wget --no-check-certificate "https://mirror.ghproxy.com/https://raw.githubusercontent.com/2H3N9HU1/doubi/master/service/server_status_client_debian" -O /etc/init.d/status-client; then
			echo -e "${Error} ServerStatus 客户端服务管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/status-client
...
Install_jq(){
	if [[ ! -e ${jq_file} ]]; then
		if [[ ${bit} = "x86_64" ]]; then
			wget --no-check-certificate "https://mirror.ghproxy.com/https://github.com/2H3N9HU1/jq/releases/download/jq-1.5/jq-linux64" -O ${jq_file}
		else
			wget --no-check-certificate "https://mirror.ghproxy.com/https://github.com/2H3N9HU1/jq/releases/download/jq-1.5/jq-linux32" -O ${jq_file}
		fi
...
Install_caddy(){
	...
	if [[ "${caddy_yn}" == [Yy] ]]; then
		...
		if [[ ! -e "/usr/local/caddy/caddy" ]]; then
			wget -N --no-check-certificate https://mirror.ghproxy.com/https://raw.githubusercontent.com/2H3N9HU1/ServerStatus-Toyo/master/caddy_install.sh
